...
<!-- All SQL queries follow a standard format. A basic version of this format is outlined below.
...
The `WHERE` clause is used to filter records based on a condition. The operators in Section \@ref(#operators) can be used to specify a condition. You can filter records based on more than one condition using the boolean operators `AND` and `OR`. Conditions can also be negated using the `NOT` operator.
...
__You should always check carefully before deleting a table and should never delete a table that you have not created yourself.__
