...
Existing guidance on the HOCAS database is avaiable [here](https://github.com/moj-analytical-services/HOCAS/blob/master/README.md).
...
if [ -d "_temp_" ]; then
  if [ -d "_temp_/nomis" ]; then
    ...
    cd _temp_
    git clone https://${GITHUB_TOKEN}@github.com/moj-analytical-services/airflow_nomis_transform.git nomis
  fi
else
  mkdir _temp_
  cd _temp_
...
table_name <- character()
table_description <- character()
for(file in list.files(path = '_temp_/nomis/meta_data/curated', full.names = TRUE)){
parsed_json <- fromJSON(file)
table_name <- append(table_name, parsed_json$name)
table_description <- append(table_description, parsed_json$description)
}
