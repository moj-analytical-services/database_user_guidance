...
```{bash clone xhibit repo, include=FALSE}
set -e
...
if [ -d "_temp_" ]; then
  if [ -d "_temp_/xhibit" ]; then
    cd _temp_/xhibit
    git pull origin master
  else
    cd _temp_
    git clone https://${GITHUB_TOKEN}@github.com/moj-analytical-services/airflow-xhibit-ap.git xhibit
  fi
else
  mkdir _temp_
  cd _temp_
